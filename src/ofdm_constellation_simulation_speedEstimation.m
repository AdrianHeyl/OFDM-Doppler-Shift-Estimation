% this file simulated the effect of doppler shift introduced by adobe
% audition, with different moving speed
% simulation setting please see the docu in wiz "simulation of OFDM" on
% 27.07.2017
...
%% setting
% by selecting flag_case & speed, you can specify the setting file & 
% wav file you want to import
...
% estimated speed, final version
v_est_diff = [];
v_est_grad = [];
deg_total = [];
deg_unwraped_total = [];
deg_unwraped_woSmooth_total = [];
w_symbol_diff = [];
w_symbol_gra = [];
...
for i_frame = 1:N_targetFrame
    ...
    %     v_end = relative_speed(v,dis,i_end);
    %     v_start = relative_speed(v,dis,i_start);
    %     disp(['relative speed of segment #',num2str(j),' is ',num2str(v_start),'-',num2str(v_end)]);
    ...
    %     figure;
    %     hold on;
    %     plot(linspace(0,Fs/2,length(BPSK_data)),abs(BPSK_data));
    %     plot(linspace(0,Fs/2,length(BPSK_demodulated)),abs(BPSK_demodulated));
...
%     deg_unwraped = smooth(unwrap(deg),'rlowess');
