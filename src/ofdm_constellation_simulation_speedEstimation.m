...
ifft_size = 2*N_sc;
bidata_len = N_sc;
cp_length = 800;  % the pre and post fix are the same length of ifft_size
...
%% check settings
disp('check settings...');
disp(['Nsc =',num2str(N_sc), ', cp-length=',num2str(cp_length)]);
disp(['Nsymbol in frame = ',num2str(N_symbol)]);
disp(['preamble:',num2str(pre_len),', fmin-fmax:',num2str(f_min),'-',num2str(f_max)]);
%% synchronization
...
disp(['diff, abs(diff) should < cp_length/2 =',num2str(cp_length/2)]);
disp(diff(index_arr)-len_frame);
